...
import { BookOpen, CheckCircle2, Circle, Lock, Brain, PlayCircle, BarChart3 } from "lucide-react";
...
export default function CourseView() {
  ...
  if (isLoading) {
    return (
      <div className="flex h-[calc(100vh-4rem)]">
        <div className="w-80 border-r border-white/10 p-6 space-y-4"><Skeleton className="h-8 w-full" /><Skeleton className="h-12 w-full" /></div>
        <div className="flex-1 p-10"><Skeleton className="h-12 w-3/4 mb-8" /><Skeleton className="h-4 w-full mb-2" /><Skeleton className="h-4 w-5/6" /></div>
      </div>
    );
  }
  ...
  const topics = [...course.topics].sort((a: any, b: any) => a.orderIndex - b.orderIndex);
  const activeTopic = topics.find(t => t.id === activeTopicId) || topics[0];
  
  const totalTopics = topics.length;
  const completedTopics = progressData?.filter((p: any) => p.isCompleted).length || 0;
    ...
    <div className="flex h-[calc(100vh-4rem)] bg-background">
      ...
      <div className="w-80 flex-shrink-0 border-r border-white/10 bg-card/30 overflow-y-auto hidden md:flex flex-col">
        <div className="p-6 border-b border-white/10 sticky top-0 bg-background/95 backdrop-blur z-10">
          <h2 className="font-display font-bold text-lg leading-tight mb-4">{course.title}</h2>
        ...
        <div className="p-4 space-y-2">
          {topics.map((topic, idx) => {
            ...
            const prevProg = idx > 0 ? progressData?.find((p: any) => p.topicId === topics[idx-1].id) : { isCompleted: true };
            ...
            return (
              <button
                ...
                className={`w-full text-left p-3 rounded-xl flex items-start gap-3 transition-all ${
                  ...
                  isUnlocked ? 'hover:bg-white/5 border border-transparent' : 'opacity-50 cursor-not-allowed border border-transparent'
                }`}
              >
                <div className="mt-0.5">
                  {isCompleted ? <CheckCircle2 className="w-5 h-5 text-green-500" /> :
                   isUnlocked ? <Circle className={`w-5 h-5 ${isActive ? 'text-primary' : 'text-muted-foreground'}`} /> :
                   <Lock className="w-5 h-5 text-muted-foreground" />}
                </div>
                <div>
                  <p className={`font-medium text-sm leading-tight ${isActive ? 'text-primary' : 'text-foreground'}`}>
                    {idx + 1}. {topic.title}
                  </p>
                  {isCompleted && prog?.difficulty && (
                    <span className="text-[10px] text-muted-foreground mt-1 block">Level: {prog.difficulty}</span>
                  )}
                </div>
              </button>
            );
        ...
        <div className="mt-auto p-6 border-t border-white/10">
          <Link href={`/progress/${course.id}`}>
            <Button variant="outline" className="w-full justify-start rounded-xl border-white/10">
      ...
      <div className="flex-1 overflow-y-auto relative scroll-smooth">
        {activeTopic ? (
          <div className="max-w-4xl mx-auto px-6 py-12 md:px-12">
            ...
            >
              ...
              <div className="mb-10">
                ...
                <div className="bg-gradient-to-r from-primary/10 to-purple-500/10 border border-white/10 rounded-2xl p-5 flex gap-4 items-start">
                  ...
                  <div>
                    ...
                    <p className="text-muted-foreground text-sm leading-relaxed">{activeTopic.aiSummary}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-16 pt-8 border-t border-white/10 flex flex-col sm:flex-row gap-4 justify-between items-center pb-20">
                <div>
                  <h4 className="font-semibold text-lg">Ready to test your knowledge?</h4>
                  <p className="text-sm text-muted-foreground">Take the adaptive quiz to unlock the next chapter.</p>
                </div>
                <Link href={`/quiz/${activeTopic.id}`}>
                  <Button size="lg" className="rounded-xl px-8 bg-primary hover:bg-primary/90 shadow-lg shadow-primary/25 text-base font-bold w-full sm:w-auto">
                    Take Chapter Quiz
                  </Button>
                </Link>
              </div>
          ...
          <div className="h-full flex items-center justify-center text-muted-foreground">Select a chapter to begin.</div>
        )}
      </div>
    </div>
  );
